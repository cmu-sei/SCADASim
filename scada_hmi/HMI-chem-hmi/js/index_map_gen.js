...
function init(r_id){
    root_id = r_id;
    $.getJSON("/api/device/tree/" + r_id, generate_map);
...
function sensor_list(data){
    for(i = 0; i < data.length; i++){
        var request = $.getJSON("/api/sensors/" + data[i].id + "/1/");
        request.done(function(data_response) {
            var dps = [];
            for (var j = 0; j < data_response.data.length; j++) {
                dps.push({x: new Date(data_response.data[j].event_time.replace(/ /g, 'T')), y: data_response.data[j].value});
            }
            add_sensor(data_response.id, dps);
        });
    }
}
...
function add_sensor(sen, datapoints) {
    ...
    var updateChart = function () {
        $.getJSON("/api/sensors/" + sen.id + "/", function (data) {
            dps.push({x: new Date(), y: data.value});
        });
