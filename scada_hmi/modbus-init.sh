...
hmi_ip=$(vmtoolsd --cmd 'info-get guestinfo.modbus.address')
port=$(vmtoolsd --cmd 'info-get guestinfo.scada.port')
host=$(vmtoolsd --cmd 'info-get guestinfo.hostname')

while [ ! -f  /var/www/HMI-$host/HMI_Server.py ]; do sleep 2; done 

/bin/python2.7 /var/www/HMI-$host/HMI_Server.py -i $hmi_ip -p $port -u admin -d sensor_network
