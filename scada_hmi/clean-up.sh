...
mv /var/www/HMI-$host /var/www/HMI-Store

hostname HMI-Template

history -c

psql -U admin -d sensor_network -c "DROP SCHEMA public CASCADE;"
psql -U admin -d sensor_network -c "CREATE SCHEMA public;"
psql -U admin -d sensor_network -c "GRANT ALL ON SCHEMA public to public;"
psql -U admin -d sensor_network -c "GRANT ALL ON SCHEMA public to admin;"

pid=($(netstat -lpn | grep 5020 | awk -F '[ /]+' '{print$7}'))
